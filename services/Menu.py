class Menu:
    ...
    def borrow_book(self):
        ...
        try:
            ...
            reservation = self.library.reserveBook(
                book_id,
                member_id,
                datetime.now(),
            ...
            if reservation and self.library.acceptReservation(reservation.id):
                if self.library.borrowBook(reservation.id):
                    self.notifier.notify("Livre emprunté avec succès!")
                else:
                    self.notifier.notify("Erreur lors de l'emprunt")
            else:
                self.notifier.notify("Erreur lors de la réservation")
    ...
    def return_book(self):
        ...
        for borrow in self.library.borrows:
            if borrow.status != "Returned":
                book = self.library.catalog.searchBookById(borrow.bookId)
    ...
    def search_book(self):
        ...
        if book:
            ...
            print(f"Disponible: {'Oui' if book.isAvailable else 'Non'}")
        else:
            print("Livre non trouvé")
